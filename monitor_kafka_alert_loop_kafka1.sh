...
# 临时文件
tmpfile=/tmp/kafka_info_tmp_kafka1.txt
...
while true;
do
    mkafka1 |awk 'NR>1 {print $2,$6}' |grep -v unknown >${tmpfile}
    ...
    for key in "${!kafka_consumer[@]}";do
        ...
        echo $num
        if [[ ${num} -gt 1000 ]];then
            messages="一区kafka总线存在积压\n主题:${key},积压消息数:${kafka_consumer[$key]}"
            zenity --error --text="${messages}" --timeout=5 --modal
        elif [[ ${num} -gt 100 ]];then
            messages="一区kafka总线存在积压\n主题:${key},积压消息数:${kafka_consumer[$key]}"
            zenity --warning --text="${messages}" --timeout=5 --modal
        elif [[ ${num} -gt 10 ]];then
            messages="一区kafka总线存在积压\n主题:${key},积压消息数:${kafka_consumer[$key]}"
            zenity --info --text="${messages}" --timeout=5 --modal
        fi
    done
