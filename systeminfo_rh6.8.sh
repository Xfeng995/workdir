...
disk_rank() {
    echo -e "\t\t\t各分区使用率\n\t挂载位置-原有空间-剩余空间-使用率"
    echo -e "\t------------------------------------------------"
    #df -hP  -x tmpfs -x devtmpfs | sort -nr -k 5 | awk '/dev/{printf "\t%-30s %10s\n", $1, $5}'
    #df -hP  -x tmpfs -x devtmpfs | sort -nr -k 5 | awk '/dev/{printf "\t%-35s %-10s %10s\n", $1, $6, $5}'
    df -hP  -x tmpfs -x devtmpfs | sort -nr -k 5 | awk '/dev/{printf "\t%-10s %10s %10s %10s\n", $6, $2, $4, $5}'
    echo
}
...
echo -e "\tUptime       : $(uptime |awk '{print "up",$3,"days", $5}' |tr -d ",")"
