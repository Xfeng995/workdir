...
disk_rank() {
    ...
    #df -h  -x tmpfs -x devtmpfs | sort -nr -k 5 | awk '/dev/{printf "\t%-20s %-10s %10s\n", $1, $6, $5}'
    df -h  -x tmpfs -x devtmpfs | sort -nr -k 5 | awk '/dev/{printf "\t%-10s %10s %10s %10s\n", $6, $2, $4, $5}'
    echo
}
...
echo -e "\tCpus         : $(lscpu  |grep '^CPU(s):' |awk '{print $2}')核"
